...
object nameBasedXml {
  ...
  class Macros(val c: whitebox.Context) extends XmlExtractor {
    ...
    private def encodedTermName(s: String) = TermName(
      encode(s.replace("$", "$u0024"))
    )

    protected class NameBasedXmlTransformer(
        vendors: Map[Option[String], Tree] = Map.empty
    ) extends Transformer {
      protected def transformAttribute(
          parentVendor: Tree,
          attributeName: QName,
          attributeValue: Tree
      ) =
        atPos(attributeValue.pos) {
          ...
          val attributeFunction =
            q"$vendor.attributes.${encodedTermName(localName(attributeName))}"
          attributeValue match {
            case TextAttributes(textValues) =>
              val transformedTexts =
                textValues.map(transformAttributeText(vendor))
      ...
      private def getVendor(prefixOption: Option[String]): Tree = {
        vendors.getOrElse(
          prefixOption,
          Ident(encodedTermName(prefixOption.getOrElse("xml")))
        )
      }

      private def resolveVendor(defaultVendor: Tree, qName: QName) =
        qName match {
          case PrefixedName(prefix, localPart) =>
      ...
      protected def transformUriText(
          factory: Tree
      ...
      protected def transformAttributeText(
          factory: Tree
      ): PartialFunction[Tree, Tree] = {
        case tree @ Text(value) =>
      ...
      protected def transformChildText(
          factory: Tree
      ): PartialFunction[Tree, Tree] = {
        case tree @ Text(value) =>
      ...
      protected def transformNamespaceBinding(
          parentVendor: Tree,
        ...
        tree match {
          case TextUris(textValues) =>
            val transformedTexts =
              textValues.map(transformUriText(parentVendor))
      ...
      protected def transformElem(
          tagName: QName,
          attributes: List[(QName, Tree)],
          children: List[Tree]
      ): Tree = {
        ...
        def transformInterpolationWithCurrentParent(tree: Tree) =
          transformInterpolation(vendor, tree)
        val transformedChildren = children.map {
          transformNode(vendor).applyOrElse(
            _,
            transformInterpolationWithCurrentParent
          )
        }
      ...
      protected def transformNode(
        ...
        transformChildText(q"$parentVendor").orElse {
          case tree @ Element(
            ...
            val transformer = if (namespaceBindings.isEmpty) {
              ...
              withNamespaces(vendors ++ namespaceBindings.view.map {
                case (prefixOption, uri) =>
                  prefixOption -> transformNamespaceBinding(
                    parentVendor,
                    prefixOption,
                    uri
                  )
            ...
            atPos(tree.pos)(
              transformer.transformElem(tagName, attributes, children)
            )
          ...
          case tree @ ProcInstr(target, data) =>
            atPos(tree.pos)(
              q"$parentVendor.processInstructions.${encodedTermName(target)}($data)"
            )
        }
      }
      ...
      protected def transformLiteral: PartialFunction[Tree, Tree] = {
        case tree @ NodeBuffer(
              transformRootNode.extract.forall(transformedNodes)
            ) =>
      ...
      protected def transformInterpolation(
          factory: Tree,
          expression: Tree
      ): Tree = {
        atPos(expression.pos)(
          q"$factory.interpolation(${super.transform(expression)})"
        )
      }
...
/** This annotation enables name based XML literal.
  *
  * All XML literals in methods that are annotated as [[nameBasedXml]] will be
  * transformed to calls to functions in `xml` object.
  *
  * {{{
  ...
  *
  * @see
  *   [[https://contributors.scala-lang.org/t/pre-sip-name-based-xml-literals/2175 Pre SIP: name based XML literals]]
  */
@compileTimeOnly("enable macro paradise to expand macro annotations")
class nameBasedXml extends StaticAnnotation {
  def macroTransform(annottees: Any*): Any =
    macro nameBasedXml.Macros.macroTransform
}
